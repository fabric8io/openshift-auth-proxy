...
function userIndex(name) { return index + '_' + name }
function userUriInsert(name) { return '_' + encodeURIComponent(name) }
...
function setupTransform(app, userForRequest) {
  var reqTransformer = function(req, res, next) {
    ...
    indexUrlsPrefixes.forEach(function(prefix) {
  	...
  	reqUrl.pathname = prefix + userUriInsert(userForRequest(req).name) +
  ...
  var responseBodyReindexer = function(req, res, next) {
    ...
    res.end = function() {
      ...
      var newIndex = userIndex(userForRequest(req).name);
      try {
